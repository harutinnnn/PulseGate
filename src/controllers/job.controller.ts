...
export default class JobController {
    ...
    try {
      ...
      if (jobData.execute_at < new Date()) {
        ...
        if (enqueued) {
          ...
          logger.warn(`Job ${createdJob.id} queue full, scheduled for later`, {
            job_id: createdJob.id,
          });
        }
      } else {
        logger.info(`Job ${createdJob.id} scheduled for ${createdJob.execute_at.toISOString()}`, {
          job_id: createdJob.id,
        });
      }
  ...
  jobs = async (req: Request, res: Response) => {
    try {
      ...
      if (!result.success) {
        return res
          .status(400)
          .json({ error: { message: 'Validation failed', details: result.error.issues } });
      }
   ...
   * @param res
   */
  job = async (req: Request<{ id: string }>, res: Response) => {
    const { id } = req.params;
   ...
   * @param res
   */
  retry = async (req: Request<{ id: string }>, res: Response) => {
    const { id } = req.params;
    ...
    if (!['failed', 'dlq'].includes(job.status)) {
      return res
        .status(409)
        .json({ error: { message: `Cannot retry job in ${job.status} state` } });
    }
  ...
  cancel = async (req: Request<{ id: string }>, res: Response) => {
    ...
    if (['processing', 'success', 'dlq', 'canceled'].includes(job.status)) {
      return res
        .status(409)
        .json({ error: { message: `Cannot cancel job in ${job.status} state` } });
    }
