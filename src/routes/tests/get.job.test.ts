...
describe('getJobHandler', () => {
  ...
  it('should return job if found', async () => {
    const jobData = parseBodyToJobData(
      {
        ...(bodyRaw as Omit<
          JobCreateDataType,
          'id' | 'created_at' | 'updated_at' | 'current_attempts'
        >),
      },
      undefined,
    );
  ...
  it('should return complete job data', async () => {
    ...
    expect({
      payload_status: returnedJob.payload_status,
      payload_order_id: returnedJob.payload_order_id,
